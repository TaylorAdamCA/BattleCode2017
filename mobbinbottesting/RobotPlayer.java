...
public strictfp class RobotPlayer {
	...
	public static void canShoot(RobotType shooter, String typeOfShot) throws GameActionException {
		...
		RobotInfo[] bots = rc.senseNearbyRobots();
		botsInLOS = 0;
		for(RobotInfo r: bots){if(r.getTeam() != rc.getTeam() && rc.getLocation().distanceTo(r.getLocation()) < RobotType.SOLDIER.bulletSightRadius) botsInLOS++;}
		if (rc.getType() == RobotType.SCOUT) {
			...
			for (RobotInfo b : bots) {
				...
				if (b.getTeam() != rc.getTeam() && shooter.canAttack()
						&& rc.getTeamBullets() >= GameConstants.SINGLE_SHOT_COST && rc.getLocation().distanceTo(b.getLocation()) > 5 && botsInLOS< 1) {
					Direction dir = rc.getLocation().directionTo(b.getLocation());
					rc.fireSingleShot(dir);
					...
					if (b.getType() == RobotType.LUMBERJACK && rc.canMove(dir.opposite())) {
						...
						&& rc.getTeamBullets() >= GameConstants.TRIAD_SHOT_COST && rc.getLocation().distanceTo(b.getLocation()) > 2.5 && rc.getLocation().distanceTo(b.getLocation()) < 5 && botsInLOS < 3) {
					Direction dir = rc.getLocation().directionTo(b.getLocation());

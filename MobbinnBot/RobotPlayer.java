...
//				if (rc.canPlantTree(plantDirList[i])
//						&& Math.abs(plantDirList[i].getAngleDegrees() - noTree.getAngleDegrees()) > 20) {
		...
		if (rc.readBroadcast(10) != 0 && rc.readBroadcast(11) != 0) {
			for (TreeInfo t : trees) {
				if (rc.isCircleOccupiedExceptByThisRobot(m, 1)) {
					...
					rc.broadcastFloat(rc.getID(), m.x);
					rc.broadcastFloat(rc.getID(), m.y);
					rc.move(go);
				}
...
//					&& rc.isCircleOccupiedExceptByThisRobot(rc.getLocation().add(dirToTry, strideRadius), (float) .5)) {

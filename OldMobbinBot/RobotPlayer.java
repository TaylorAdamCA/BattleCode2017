...
public strictfp class RobotPlayer {
	...
	static MapLocation[] checkFor6 = new MapLocation[6];
	static ArrayList<Integer> arrFoundSpot = new ArrayList<Integer>();
	...
	public static int checkFor6() throws GameActionException {
		int howMany = 0;
		MapLocation ourArchon = new MapLocation(0, 0);
		MapLocation myLocation = rc.getLocation();
		if (!arrFoundSpot.contains(rc.getID())) {
			howMany = 0;
			for (int i = 0; i < 6; i++) {
				RobotInfo[] bots = rc.senseNearbyRobots();
				for (RobotInfo b : bots) {
					if (b.getTeam() == rc.getTeam() && rc.getType() == RobotType.ARCHON)
						ourArchon = rc.getLocation();
				}
				float radians = (float) (-Math.PI + 2 * Math.PI * ((float) i / 6));
				checkFor6[i] = new MapLocation(myLocation.x, myLocation.y).add(radians).add(radians);
				rc.setIndicatorDot(checkFor6[i], 000, 111, 222);
				if (!rc.isCircleOccupiedExceptByThisRobot(checkFor6[i], (float) 1)
						&& rc.getLocation().distanceTo(ourArchon) > 5)
					howMany++;
			}
			if (howMany > 5)
				arrFoundSpot.add(rc.getID());
			else {
	...
	public static void runArchon() {
		while (true) {
			try {
				 ...
				 */
				if (rc.getRoundNum() < 50 || rc.getRoundNum() % 100 == 0)
				...
				/*
				 * archon = 0; fourGard = 0; RobotInfo[] bots =
				 * rc.senseNearbyRobots(); for (RobotInfo b : bots) { if
				 * (b.getTeam() != rc.getTeam()) shouldIDodoge++; if
				 * (b.getType() == RobotType.GARDENER) fourGard++;
				 * 
				 * } if (fourGard == 3 && archonGoTest == false) { for (int x =
				 * 0; x < 8; x++) { if (rc.canMove(dirList[x])) archonGo =
				 * dirList[x]; } } if (fourGard == 3) if (rc.canMove(archonGo)
				 * && archonMoved < 8) { rc.move(archonGo); archonMoved++; }
				 */
	...
	private static boolean pathFinder(MapLocation target, float strideRadius) throws GameActionException {
		...
		for (int i = 0; i < toTry.length; i++) {
			...
			if (rc.canMove(dirToTry, strideRadius)
					&& rc.isCircleOccupiedExceptByThisRobot(rc.getLocation().add(dirToTry, strideRadius), (float) .1)) {
	...
	private static boolean moveToTarget(MapLocation location) throws GameActionException{
        ...
        if (pathFinder(location, rc.getType().strideRadius)) {
            return(true);
        }
        // try to take a smaller step
        if (pathFinder(location, rc.getType().strideRadius/2)) {
            return(true);
        }
        // try to take a baby step
        if (pathFinder(location, rc.getType().strideRadius/4)) {
            return(true);
        }
        else {
            rc.move(goingDir);
